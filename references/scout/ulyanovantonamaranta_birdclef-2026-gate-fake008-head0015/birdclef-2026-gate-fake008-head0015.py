...
# 4) low-weight train_audio linear-head rank signal
HEAD_RANK_BLEND = 0.0500
head_candidates = sorted(glob.glob("/kaggle/input/**/head_weights_train_audio.npz", recursive=True))
if not head_candidates:
    raise FileNotFoundError("head_weights_train_audio.npz not found. Attach konbu17/bird26-train-audio-head-v1.")
...
assert emb_te.shape[1] == head_W.shape[1], f"head dim mismatch: emb_te={emb_te.shape[1]} W={head_W.shape[1]}"
...
pred = (1.0 - HEAD_RANK_BLEND) * pred + HEAD_RANK_BLEND * xc
print(f"Applied train_audio head rank blend: {HEAD_RANK_BLEND:.4f}")
