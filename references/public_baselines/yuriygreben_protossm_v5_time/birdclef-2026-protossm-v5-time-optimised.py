...
TF_WHEELS_HINT = "tf-wheels"  # e.g. "tf-wheels" or "bc26-tensorflow-2-20-0"
...
PERCH_MODEL_HINT = "perch_v2_cpu"  # e.g. "perch_v2_cpu"
PERCH_CACHE_HINT = "perch-meta"  # e.g. "perch-meta"
...
CFG = {
    ...
    "best_fusion": {"lambda_event": 0.4, "lambda_texture": 1.0, "lambda_proxy_texture": 0.8, "smooth_texture": 0.35, "smooth_event": 0.15},
    "proto_ssm": {"d_model": 128, "d_state": 16, "n_ssm_layers": 2, "dropout": 0.15, "n_prototypes": 1, "n_sites": 20, "meta_dim": 16, "use_cross_attn": True, "cross_attn_heads": 4},
    "proto_ssm_train": {"n_epochs": 60, "lr": 1e-3, "weight_decay": 2e-3, "val_ratio": 0.15, "patience": 15, "pos_weight_cap": 30.0, "distill_weight": 0.1, "proto_margin": 0.1, "label_smoothing": 0.02, "oof_n_splits": 3, "mixup_alpha": 0.3, "focal_gamma": 2.0, "swa_start_frac": 0.7, "swa_lr": 5e-4},
    "frozen_best_probe": {"pca_dim": 64, "min_pos": 8, "C": 0.50, "alpha": 0.40},
    "residual_ssm": {"d_model": 64, "d_state": 8, "n_ssm_layers": 1, "dropout": 0.1, "correction_weight": 0.3, "n_epochs": 30, "lr": 1e-3, "patience": 8},
    ...
    "threshold_grid": np.array([0.20, 0.30, 0.40, 0.50, 0.60, 0.70], dtype=np.float32),
    "probe_backend": "mlp",
    "mlp_params": {
...
# %% cell 7
# NEW: Focal loss, file-level scaling, TTA, rank-aware, delta shift, per-class thresholds
...
if cache_meta is not None and cache_npz is not None:
    ...
    OOF_META_CACHE.parent.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(
        out_npz,
...
if CFG["run_probe_grid"]:
    ...
    best_probe_path.parent.mkdir(parents=True, exist_ok=True)
...
if grid_results is not None:
    grid_results_path = CFG["full_cache_work_dir"] / "probe_grid_results.csv"
