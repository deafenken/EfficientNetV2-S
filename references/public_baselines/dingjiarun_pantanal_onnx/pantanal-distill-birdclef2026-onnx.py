...
def temporal_shift_tta(emb_files, logits_files, model, site_ids, hours, shifts=[0, 1, -1]):
    ...
    for shift in shifts:
        ...
        with torch.no_grad():
            ...
            # 别忘了这里也要加 .cpu()，否则后面又会报 numpy 转换错误
            pred = out.cpu().numpy()
        # with torch.no_grad():
        #     out, _, _ = model(
...
def train_proto_ssm_single(model, emb_train, logits_train, labels_train,
    ...
    if has_val:
        ...
        site_v = torch.tensor(site_ids_val, dtype=torch.long).to(DEVICE) if site_ids_val is not None else None
        hour_v = torch.tensor(hours_val, dtype=torch.long).to(DEVICE) if hours_val is not None else None

    fam_v = torch.tensor(file_families_val, dtype=torch.float32).to(DEVICE) if (has_val and file_families_val is not None) else None
    ...
    for epoch in range(n_epochs):
        ...
        site_tr = torch.tensor(site_ids_train, dtype=torch.long).to(DEVICE) if site_ids_train is not None else None
        hour_tr = torch.tensor(hours_train, dtype=torch.long).to(DEVICE) if hours_train is not None else None
...
emb_flat_tensor = torch.tensor(emb_full, dtype=torch.float32).to(DEVICE)
labels_flat_tensor = torch.tensor(Y_FULL, dtype=torch.float32).to(DEVICE)
...
with torch.no_grad():
    ...
    top_sims = cos_sim.max(dim=1)[0].cpu().numpy()
...
# 释放 TF 占用的显存 (虽然不能完全释放，但能缓解)
tf.keras.backend.clear_session()
# 关键：尝试通过 gc 回收
gc.collect()

# 如果你是在双 T4 环境，让 PyTorch 换到另一张卡上试试
...
# --- 极小 Batch 修复版 Step 4 ---
batch_size_ssm = 100  # 降到 100，确保万无一失
proto_pseudo_scores_list = []
model.eval()

# 再次清理
torch.cuda.empty_cache()

print(f"[*] 启动极小 Batch 推理...")
for i in tqdm(range(0, len(emb_pseudo_files), batch_size_ssm), desc="Final Push"):
    e_chunk = emb_pseudo_files[i : i + batch_size_ssm]
    l_chunk = logits_pseudo_files[i : i + batch_size_ssm]
    s_chunk = pseudo_site_ids[i : i + batch_size_ssm]
    h_chunk = pseudo_hours[i : i + batch_size_ssm]
    
    try:
        chunk_pred = temporal_shift_tta(
            e_chunk, l_chunk, model, s_chunk, h_chunk, 
            shifts=CFG.get("tta_shifts", [0])
        )
        proto_pseudo_scores_list.append(chunk_pred)
    except RuntimeError as e:
        if "out of memory" in str(e):
            print(f"[!] 仍然 OOM, 尝试在 CPU 推理该 block...")
            # 最后的保命手段：如果显存还是爆，这个小块切到 CPU 跑
            torch.cuda.empty_cache()
            # 临时把 model 移到 CPU
            model.cpu()
            # CPU 推理 (注意 DEVICE 要临时换成 'cpu')
            old_device = DEVICE
            DEVICE = torch.device("cpu")
            chunk_pred = temporal_shift_tta(e_chunk, l_chunk, model, s_chunk, h_chunk, shifts=CFG.get("tta_shifts", [0]))
            proto_pseudo_scores_list.append(chunk_pred)
            # 切回来
            DEVICE = old_device
            model.to(DEVICE)
...
proto_pseudo_scores = np.concatenate(proto_pseudo_scores_list, axis=0)
print("[√] 终于搞定了！继续下一步。")

# %% cell 35
# ── BIRD_CLEF 2026: 超级集成伪标签生成引擎 (全量/修复/根目录保存版) ──
...
model = model.to(DEVICE)
if 'res_model' in locals() and res_model is not None:
    res_model = res_model.to(DEVICE)

# 1. 搜寻目标音频 (10658个文件)
pseudo_paths = sorted((BASE / "train_soundscapes").glob("*.ogg"))
print(f"[*] 目标文件数: {len(pseudo_paths)}")

# 2. Perch 特征提取 (这一步如果已经跑过且 emb_pseudo 在内存里，会很快跳过)
# 注意：如果重启了 Kernel，请确保这一步完整运行
print("[*] Step 1: 提取 Perch Embeddings...")
meta_pseudo, scores_pseudo_raw, emb_pseudo = infer_perch_with_embeddings(
    pseudo_paths,
    batch_files=CFG["batch_files"],
    verbose=True,
    proxy_reduce=CFG["proxy_reduce"],
...
# 3. 数据形状对齐与重组
print("[*] Step 2: 重组序列数据...")
emb_pseudo_files, pseudo_file_list = reshape_to_files(emb_pseudo, meta_pseudo)
logits_pseudo_files, _ = reshape_to_files(scores_pseudo_raw, meta_pseudo)
pseudo_site_ids, pseudo_hours = get_file_metadata(
    meta_pseudo, pseudo_file_list, site_to_idx, CFG["proto_ssm"]["n_sites"]
)

# 4. 分块 ProtoSSM 推理 (解决 OOM)
print("[*] Step 3: 分块运行 ProtoSSM 推理 (带 TTA)...")
batch_size_ssm = 100  # 极小 batch 确保显存安全
proto_pseudo_list = []
tta_shifts = CFG.get("tta_shifts", [0])

for i in tqdm(range(0, len(emb_pseudo_files), batch_size_ssm), desc="ProtoSSM Batch"):
    e_chunk = emb_pseudo_files[i : i + batch_size_ssm]
    l_chunk = logits_pseudo_files[i : i + batch_size_ssm]
    s_chunk = pseudo_site_ids[i : i + batch_size_ssm]
    h_chunk = pseudo_hours[i : i + batch_size_ssm]
    ...
    for shift in tta_shifts:
        e_roll = np.roll(e_chunk, shift, axis=1) if shift != 0 else e_chunk
        l_roll = np.roll(l_chunk, shift, axis=1) if shift != 0 else l_chunk
        
        with torch.no_grad():
            out, _, _ = model(
                torch.tensor(e_roll, dtype=torch.float32).to(DEVICE),
                torch.tensor(l_roll, dtype=torch.float32).to(DEVICE),
                site_ids=torch.tensor(s_chunk, dtype=torch.long).to(DEVICE),
                hours=torch.tensor(h_chunk, dtype=torch.long).to(DEVICE)
            )
            pred = out.cpu().numpy()
            if shift != 0:
                pred = np.roll(pred, -shift, axis=1)
    ...
    proto_pseudo_list.append(np.mean(all_tta_preds, axis=0))
    if i % 500 == 0:
        torch.cuda.empty_cache()
        gc.collect()

proto_scores_flat = np.concatenate(proto_pseudo_list, axis=0).reshape(-1, N_CLASSES).astype(np.float32)

# 5. MLP 探针打分 (CPU 运行)
print("[*] Step 4: 运行 MLP 探针...")
pseudo_base_scores, pseudo_prior_scores = fuse_scores_with_tables(
    scores_pseudo_raw,
    sites=meta_pseudo["site"].to_numpy(),
    hours=meta_pseudo["hour_utc"].to_numpy(),
    tables=final_prior_tables,
)
Z_PSEUDO = emb_pca.transform(emb_scaler.transform(emb_pseudo)).astype(np.float32)
mlp_pseudo_scores = pseudo_base_scores.copy()

for cls_idx, clf in tqdm(probe_models.items(), desc="Probes"):
    X_cls = build_class_features(Z_PSEUDO, scores_pseudo_raw[:, cls_idx], 
                                 pseudo_prior_scores[:, cls_idx], pseudo_base_scores[:, cls_idx])
    if hasattr(clf, "predict_proba"):
        p = clf.predict_proba(X_cls)[:, 1].astype(np.float32)
        pred = np.log(p + 1e-7) - np.log(1 - p + 1e-7)
    else:
        pred = clf.decision_function(X_cls).astype(np.float32)
    mlp_pseudo_scores[:, cls_idx] = (1.0 - float(CFG["frozen_best_probe"]["alpha"])) * pseudo_base_scores[:, cls_idx] + float(CFG["frozen_best_probe"]["alpha"]) * pred

# 6. 集成与残差修正
print("[*] Step 5: 最终融合与 Residual 修正...")
final_pseudo_scores = (ENSEMBLE_WEIGHT_PROTO * proto_scores_flat + (1.0 - ENSEMBLE_WEIGHT_PROTO) * mlp_pseudo_scores)

if 'res_model' in locals() and res_model is not None:
    fp_files, _ = reshape_to_files(final_pseudo_scores, meta_pseudo)
    res_list = []
    for i in range(0, len(emb_pseudo_files), 500):
        with torch.no_grad():
            c = res_model(torch.tensor(emb_pseudo_files[i:i+500]).to(DEVICE), 
                          torch.tensor(fp_files[i:i+500]).to(DEVICE),
                          site_ids=torch.tensor(pseudo_site_ids[i:i+500]).to(DEVICE),
                          hours=torch.tensor(pseudo_hours[i:i+500]).to(DEVICE)).cpu().numpy()
            res_list.append(c)
    final_pseudo_scores += CORRECTION_WEIGHT * np.concatenate(res_list, axis=0).reshape(-1, N_CLASSES)

# 7. 降噪 (Power Transform) 与 保存
print("[*] Step 6: 执行降噪并保存到根目录...")
probs = 1.0 / (1.0 + np.exp(-np.clip(final_pseudo_scores / class_temperatures[None, :], -30, 30)))
probs = np.power(np.clip(probs, 0.0, 1.0), 2.0) # Power 2.0 降噪

pseudo_df = pd.DataFrame(probs, columns=PRIMARY_LABELS)
pseudo_df.insert(0, "row_id", meta_pseudo["row_id"].values)
pseudo_df['filename'] = meta_pseudo["filename"].values
pseudo_df['end_sec'] = pseudo_df['row_id'].apply(lambda x: int(x.split('_')[-1]))
pseudo_df['start_sec'] = pseudo_df['end_sec'] - 5
pseudo_df['max_prob'] = probs.max(axis=1)

# 直接保存到 Kaggle 根目录，防止找不到
FINAL_PATH = "/kaggle/working/proto_pseudo_v18_final.csv"
pseudo_df.to_csv(FINAL_PATH, index=False)

print(f"\n[√] 全部完成！请在右侧 Output 栏直接刷新，下载文件: {FINAL_PATH}")
