...
def train_proto_ssm_single(model, emb_train, logits_train, labels_train,
    ...
    for epoch in range(n_epochs):
        ...
        loss_distill = F.mse_loss(species_out, logits_tr.detach())
...
# V18: Isotonic calibration
from sklearn.isotonic import IsotonicRegression

def apply_isotonic_calibration(probs, Y_FULL, n_windows=12):
    """Apply isotonic calibration per class."""
    n_samples, n_cls = probs.shape
    n_files = n_samples // n_windows
    file_probs = probs.reshape(n_files, n_windows, n_cls).max(axis=1)
    file_y = Y_FULL.reshape(n_files, n_windows, n_cls).max(axis=1)
    
    calibrated = np.zeros_like(probs)
    for c in range(n_cls):
        y_true, y_prob = file_y[:, c], file_probs[:, c]
        if y_true.sum() < 3:
        ...
        try:
            ...
            cal_val = ir.transform(y_prob)
            for f in range(n_files):
                calibrated[f*n_windows:(f+1)*n_windows, c] = cal_val[f]
        except:
            calibrated[:, c] = probs[:, c]
    return calibrated

# Isotonic calibration only works in train mode with OOF data
# Skip in submit mode as test data doesn't have labels
if 'oof_proto_flat' in dir() and oof_proto_flat is not None:
    print("Applying isotonic calibration (train mode)...")
    probs = apply_isotonic_calibration(probs, Y_FULL, n_windows=N_WINDOWS)
else:
    print("Skipping isotonic calibration (submit mode)")
