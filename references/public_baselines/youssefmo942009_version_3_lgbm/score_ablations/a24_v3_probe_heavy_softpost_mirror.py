...
print("A24 config: proto=0.40 residual=0.00 rank_power=0.35 delta=0.10 threshold=off mirror=on")
...
def build_class_features(emb_proj, raw_col, prior_col, base_col,
                          hour_utc=None, site_id=None, window_idx=None):
    
    # --- existing seq features ---
    ...
    # ── NEW 1: Time-of-day context (EDA §2: 65.9% night, dawn/dusk peaks)
    # Species activity is strongly time-gated — encode this directly
    if hour_utc is not None:
        ...
        parts += [
            ...
            is_dusk[:,  None],
            is_night[:, None],
            # interactions: score × time-of-day (nocturnal species score higher at night)
            (raw_col * is_night)[:, None],
            (raw_col * is_dawn )[:, None],
        ]

    # ── NEW 2: Within-file window position (EDA §8: activity peaks at window 8)
    # Helps distinguish transient calls vs sustained chorus
    if window_idx is not None:
        n = len(base_col)
        win_norm  = (window_idx / 11.0).astype(np.float32)   # 0→1
        win_early = (window_idx <= 2).astype(np.float32)
        win_late  = (window_idx >= 9).astype(np.float32)
        # "Is this window the peak for this class in this file?"
        is_peak = (base_col == max_base).astype(np.float32)
        parts += [
            win_norm[:,  None],
            win_early[:, None],
            win_late[:,  None],
    ...
    # ── NEW 3: Score distribution features (EDA §7: logit range −7.7 to +12.8)
    # Raw logits benefit from rank/percentile context across the 234-class axis
    # (compute these per-window across all classes before calling this function)
    # Pass in: rank of this class, z-score, softmax value
    # Example pre-computation (outside this fn, for all 234 classes):
    #   ranks      = scipy.stats.rankdata(scores_row) / 234        # (234,)
    #   z_scores   = (scores_row - scores_row.mean()) / scores_row.std()
    #   softmax    = np.exp(scores_row) / np.exp(scores_row).sum()

    # ── NEW 4: Score momentum — EDA shows ~42.5% values > 1.0 (strong logits)
    # Capture whether the score is rising or falling across the file
    score_range  = max_base - base_col          # distance from peak
    score_accel  = diff_prev - diff_next        # second-order difference
    parts += [
        score_range[:, None],
        score_accel[:, None],
        # Clip-aware interaction: only trust high raw scores
        np.clip(raw_col, 0, None)[:, None],     # ReLU of raw logit
        (raw_col ** 2)[:, None],                # quadratic — captures extremes
    ]

    # ── NEW 5: Site-level prior (EDA §9: S22 dominates with 468/708 windows)
    # Smoothed species rate per site to handle small-site sparsity (<5 files)
    # Pass in site_prior_col: per-row smoothed P(species | site)
    # site_prior_col = site_species_smooth[site_id, class_idx]  # precomputed
    # parts.append(site_prior_col[:, None])
    # parts.append((raw_col * site_prior_col)[:, None])
...
# %% cell 15
# Phase 5: Train MLP + LGBM probes (ensemble)
print("Phase 5: Train MLP + LGBM probes (ensemble)...")
t0 = time.time()
...
# Train final MLP + LGBM probes (ensemble 60 MLP / 40 LGBM)
PROBE_CLASS_IDX = np.where(Y_FULL.sum(axis=0) >= int(CFG["frozen_best_probe"]["min_pos"]))[0].astype(np.int32)
probe_models = {}   # cls_idx -> {"mlp": clf_mlp, "lgbm": clf_lgbm}

W_MLP  = CFG["probe_ensemble_weights"]["mlp"]   # 0.60
W_LGBM = CFG["probe_ensemble_weights"]["lgbm"]  # 0.40

lgbm_base_params      = {k: v for k, v in CFG["lgbm_params"].items()
                          if k not in ("n_estimators", "early_stopping_rounds")}
n_estimators          = CFG["lgbm_params"]["n_estimators"]
early_stopping_rounds = CFG["lgbm_params"]["early_stopping_rounds"]

from sklearn.model_selection import train_test_split

for cls_idx in tqdm(PROBE_CLASS_IDX, desc="Training MLP+LGBM probes"):
    y = Y_FULL[:, cls_idx].astype(np.float32)
    ...
    if n_pos > 0 and n_neg > n_pos:
        ...
        X_bal, y_bal = X_cls, y
    clf_mlp = MLPClassifier(**CFG["mlp_params"])
    clf_mlp.fit(X_bal, y_bal)

    # ── LGBM ─────────────────────────────────────────────────────────────────
    scale_pos_weight = n_neg / max(n_pos, 1)
    X_tr, X_val, y_tr, y_val = train_test_split(
        X_cls, y, test_size=0.15, random_state=42, stratify=y if n_pos >= 2 else None
    )
...
print(f"  Done. Probe pairs trained: {len(probe_models)}  (MLP×{W_MLP} + LGBM×{W_LGBM})")
print(f"  Phase 5: {time.time() - t0:.1f}s")


# %% cell 16
# Probe ensemble predict helper
# ─────────────────────────────────────────────────────────────────────────────
def probe_predict_logit(clf_pair, X):
    """Average logit from MLP (60%) + LGBM (40%)."""
    W_MLP  = CFG["probe_ensemble_weights"]["mlp"]
    W_LGBM = CFG["probe_ensemble_weights"]["lgbm"]

    def to_logit(clf, X_):
        prob = clf.predict_proba(X_)[:, 1].astype(np.float32)
        return np.log(prob + 1e-7) - np.log(1 - prob + 1e-7)

    return W_MLP * to_logit(clf_pair["mlp"], X) + W_LGBM * to_logit(clf_pair["lgbm"], X)
...
# MLP+LGBM ensemble probe scores on test
...
for cls_idx, clf_pair in probe_models.items():
    ...
    pred = probe_predict_logit(clf_pair, X_cls_test)
...
for cls_idx, clf_pair in probe_models.items():
    ...
    pred = probe_predict_logit(clf_pair, X_cls)
    alpha_p = float(CFG["frozen_best_probe"]["alpha"])
...
# A24 sonotype mirroring: conservative transfer from the needless090 reference.
...
# %% cell 20
# Ensemble accuracy on dry-run (train soundscapes used as test)
if not RUN_INFERENCE:
    try:
        # Match test windows to cached ground truth via row_id
        ...
        if matched.sum() > 0:
            ...
            Y_test_true = Y_FULL[full_positions]
            probs_matched = probs[matched]
            ensemble_auc = macro_auc_skip_empty(Y_test_true, probs_matched)
            print(f"\nEnsemble macro-AUC (dry-run, {matched.sum()} windows): {ensemble_auc:.4f}")
        else:
            print("\nEnsemble AUC: no matching row_ids found between test and cached data.")
    except Exception as e:
        print(f"\nEnsemble AUC could not be computed: {e}")
