...
solutions = {
  'Models': [
       {f'Model':'Model_2', f'subm':'subm_2.csv', f'weight':0.333, f'LB': 0.928},
       {f'Model':'Model_3', f'subm':'subm_3.csv', f'weight':0.333, f'LB': 0.931},
       {f'Model':'Model_1', f'subm':'subm_1.csv', f'weight':0.001, f'LB': 0.720},
       {f'Model':'Model_5', f'subm':'subm_5.csv', f'weight':0.333, f'LB': 0.931},
  ]
}
...
# %% cell 3
if 'Model_1' in _ensemble_models:
    ...
    class BirdModel(nn.Module):
        def __init__(self, config=None):
            ...
            self.config = {
                ...
                'backbone': 'mobilenetv3_large_100',  # Changed architecture
                'dropout': 0.1,
                'pretrained': False,
                'channels': 1,
                'num_labels': 234,
    ...
    class BirdDataset(Dataset):
        ...
        def __init__(self):
            ...
            # Fallback to train data if no test files found (for local testing/debugging)
            if len(paths) == 0:
                paths = sorted(
                    [self.TRAIN_PATH + x for x in os.listdir(self.TRAIN_PATH) if '.ogg' in x]
                )[:16]
    ...
    def format_time(t):
        ...
        m = int(t / 60);    t -= m * 60
        prefix = f"{h}:" if h > 0 else ""
        return f"{prefix}{str(m).zfill(2)}:{str(int(t)).zfill(2)}"
    ...
    def load_model(exp_id: str, cfg: dict) -> BirdModel:
        model_path = (
            f"/kaggle/input/notebooks/stpeteishii/gpu-birdclef-2026-mobilenetv3-train"
            f"/models/{exp_id}.pth"
        )
    ...
    def predict(filepath: str) -> tuple[np.ndarray, list[str]]:
        """
        Loads .ogg file using librosa, splits a 60-second clip into 12 segments 
        (5s each), and performs inference. Short files are zero-padded.
        """
        SR    = dataset.SR
        DUR   = dataset.DUR
        n_seg = int(60 / DUR)                     # 12 segments
        total = SR * 60                           # 1,920,000 samples
    
        # Load audio with librosa (supports .ogg)
        wav, _ = librosa.load(filepath, sr=SR, mono=True, duration=60.0)
        wav    = torch.tensor(wav).float()
    
        # Zero-padding if clip is shorter than 60s
        if len(wav) < total:
            pad = torch.zeros(total)
            pad[:len(wav)] = wav
            wav = pad
        else:
            wav = wav[:total]
    
        wav = wav.reshape(n_seg, SR * DUR)        # Shape: (12, 160000)
    
        with torch.no_grad():
            mel  = torch.stack([spec(wav[i]) for i in range(n_seg)])
            pred = torch.sigmoid(model(mel)).numpy()
    
        stem  = os.path.splitext(os.path.basename(filepath))[0]
        ends  = (np.arange(n_seg) * DUR + DUR).astype(str)
        names = [f"{stem}_{t}" for t in ends]
    ...
    exp_id  = '949a67b2b3ddef89be02cb48cf8bc52c1482b4d045ec66b920d84b0813e9827b'
    history = pd.read_csv(
        f"/kaggle/input/notebooks/stpeteishii/gpu-birdclef-2026-mobilenetv3-train"
        f"/history/{exp_id}.csv"
    )
    
    cfg = decode_config({k: history.iloc[0][k] for k in history.columns[7:]})
    cfg['pretrained'] = False
    
    spec  = Spectrogram(**cfg['mel'])
    model = load_model(exp_id, cfg)
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        for p, n in executor.map(predict, dataset.paths):
            ...
            fps  = done / (time() - start)
            if done % 16 == 0:
                remaining = (len(dataset) - done) / fps
                print(
                    f"{round(100 * done / len(dataset))} % "
    ...
    # ── Build Submission ──────────────────────────────────────────────────────────
    Pred = pd.DataFrame(
        np.zeros((len(all_pred), len(dataset.LABELS))),
        columns=dataset.LABELS,
    )
    Pred[LABELS_audio] = all_pred
    Pred.insert(0, 'row_id', np.concatenate(all_names, axis=0))
    Pred.to_csv('subm_1.csv', index=False)
    print("submission.csv saved:", Pred.shape)
    
    
    # ── Visualization ─────────────────────────────────────────────────────────────
    def visualize(i: int):
        filepath = dataset.paths[i]
        wav, _   = librosa.load(filepath, sr=dataset.SR, mono=True)
        wav      = torch.tensor(wav).float()
        mel      = spec(wav, resize=False)[0]   # shape: (n_mels, time)
    
        n_seg    = int(60 / dataset.DUR)
        seg_pred = all_pred[i * n_seg:(i + 1) * n_seg]
    
        fig = px.imshow(
            seg_pred.T,
            color_continuous_scale='Oranges',
            aspect='auto', zmax=1,
            title=f"Predictions – {os.path.basename(filepath)}",
            labels={"x": "Segment", "y": "Species"},
        )
...
if 'Model_2' in _ensemble_models:
    ...
    submission.to_csv("subm_2.csv", index=False)
...
if 'Model_5' in _ensemble_models:
    ...
    def calibrate_and_optimize_thresholds(oof_probs, Y_FULL, 
                                           threshold_grid=None, n_windows=12):
        if threshold_grid is None:
            threshold_grid = np.arange(0.15, 0.76, 0.02) 
...
# %% cell 9
def direct_add():
    if len(_ensemble_models) == 2:  return add2()
    if len(_ensemble_models) == 3:  return add3()
    if len(_ensemble_models) == 4:  return add4()

# %% cell 10
submission = direct_add()
