# %% cell 1
# Cell 0 — Install ONNX Runtime (replaces TensorFlow for Perch inference, 9x faster)
import os, glob
...
# Still need TF for labels.csv mapping only (already in base image)
!pip install -q --no-deps /kaggle/input/tensorflow-2-20-wheels/tensorboard-2.20.0-py3-none-any.whl 2>/dev/null || true
!pip install -q --no-deps /kaggle/input/tensorflow-2-20-wheels/tensorflow-2.20.0-cp312-cp312-manylinux_2_17_x86_64.manylinux2014_x86_64.whl 2>/dev/null || true
...
# ONNX Perch model (9x faster than TF on CPU)
_onnx_candidates = [
    Path("/kaggle/input/birdclef26-perch-onnx/perch_v2_no_dft.onnx"),
...
CFG = {
    ...
    # ProtoSSM: base config (overridden by CFG upgrades cell below)
    "proto_ssm": {
        "d_model": 256,               # base d_model (upgraded to 320 in CFG upgrades)
        "d_state": 16,
        "n_ssm_layers": 3,            # base n_ssm_layers (upgraded to 4 in CFG upgrades)
        "dropout": 0.15,
        "n_prototypes": 1,
        "n_sites": 20,
        "meta_dim": 16,
    ...
    # Per-class thresholds (grid search range)
...
print("Base CFG loaded (ProtoSSM d_model=256, n_ssm_layers=3; overridden by CFG upgrades cell)")
print(json.dumps(
...
# %% cell 4
# ── CFG Upgrades: ProtoSSM d_model=320, 4 SSM layers, 5-fold OOF, cosine restarts ──
CFG["proto_ssm"] = {
    "d_model": 256, "d_state": 32, "n_ssm_layers": 3,
    "dropout": 0.12, "n_prototypes": 2, "n_sites": 20,
    "meta_dim": 24, "use_cross_attn": True, "cross_attn_heads": 8,
}
CFG["proto_ssm_train"] = {
    "n_epochs": 70, "lr": 8e-4, "weight_decay": 1e-3,
...
CFG["residual_ssm"] = {
    ...
    "n_epochs": 25, "lr": 8e-4, "patience": 12,
}
...
# %% cell 14
# Post-processing utilities: Focal loss, file-level scaling, TTA, rank-aware, delta shift, per-class thresholds
...
# TTA — average predictions from shifted temporal sequences
model.eval()
tta_shifts = CFG.get("tta_shifts", [0])
if len(tta_shifts) > 1:
...
# %% cell 31
# Cell 18 — Full post-processing pipeline

# Optimize per-class thresholds from OOF (train mode only)
...
# --- Step 3: Rank-aware post-processing ---
