# %% cell 1
# =========================================================
# BirdCLEF 2026 Improved Pipeline - ONNX fixed
# No third-party feature cache / no other-user generated feature datasets.
#
# Why this version exists:
#   The official TF SavedModel can fail on Kaggle Latest Container with
#   StableHLO / VHLO errors such as `vhlo.cosine_v2`.
#   Therefore this notebook uses ONNX Perch for inference by default.
#
# Attach Kaggle inputs:
#   1. birdclef-2026 competition data
#   2. official Google Perch model
#   3. rishikeshjani/perch-onnx-for-birdclef-2026
#
# Removed for safety:
#   - jaejohn/perch-meta
#   - any other third-party Perch feature cache
# =========================================================
print("ONNX fixed mode: no third-party Perch feature cache is used.")
...
# Install ONNX Runtime from the attached ONNX dataset.
ONNX_WHL = find_file("onnxruntime-*.whl")
if ONNX_WHL is not None:
    subprocess.run([sys.executable, "-m", "pip", "install", "-q", "--no-deps", str(ONNX_WHL)], check=True)
    print("ONNX Runtime installed:", ONNX_WHL)
else:
    print("WARNING: ONNX Runtime wheel not found. Attach rishikeshjani/perch-onnx-for-birdclef-2026.")
...
try:
    ...
    print("ONNX Runtime unavailable:", repr(e))

USE_ONNX = _ONNX_AVAILABLE
...
print("Config ready")
print("Safety changes: no third-party feature cache; ONNX inference enabled; prior/MLP/residual/postprocess weakened.")
...
# IMPORTANT:
#   Prefer ONNX because the TF SavedModel often fails in Kaggle Latest Container
#   with StableHLO / VHLO deserialization errors.
ONNX_PERCH_PATH = find_file("perch_v2.onnx")
USE_ONNX = bool(_ONNX_AVAILABLE and ONNX_PERCH_PATH is not None)
...
if USE_ONNX:
    ...
    print("ONNX Perch not available. Falling back to TF SavedModel; this may fail on Latest Container.")
...
EXTERNAL_CACHE_DIRS = []  # safe mode: no third-party cache paths
...
# ── Priority: local working > build only ─────────────────────────────
...
def apply_prior(scores, sites, hours, tables, lambda_prior=0.25):
    """
    Add a scaled prior logit to the raw Perch scores.
    
    lambda_prior=0: no effect (your baseline)
    lambda_prior=0.25: moderate influence from location/time
...
probe_models, emb_scaler, emb_pca, alpha_blend = train_mlp_probes(
    ...
    min_pos=5, pca_dim=64, alpha_blend=0.25,
)
...
# Safer, lighter post-processing.
# Avoid stacking multiple aggressive scalers because they can crush true peaks.
...
# This transform is monotonic per class in most cases, but keep it switchable.
APPLY_CLASS_THRESHOLDS = True
if APPLY_CLASS_THRESHOLDS:
