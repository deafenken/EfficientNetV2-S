# %% [markdown]
# # BirdCLEF 2026 — b07: c0 deep ensemble (A34 + NFNet 4-way RRF)
#
# Fork of A34 (Konbu Head + Mattia rescue + ProtoSSM, LB 0.942-0.943) plus
# our eB1b NFNet-L0 (LB 0.798) as a 4th branch, blended via Reciprocal Rank
# Fusion with per-class OOF-AUC weighting and 5 disagreement rescue rules.
#
# Architecture diversity is the point: Proto / SED / Head are all BN-heavy
# CNNs; NFNet-L0 is normalizer-free (no BN running stats), so its errors
# under XC→soundscape domain shift are uncorrelated with the other three.
#
# Strict-fail mode: any failure (timm install, ckpt load, row count
# mismatch) crashes the kernel. No fail-soft, no zero-stub fallback.
# That's by design — we want a clean signal from the LB, not a degraded
# A34-only blend masquerading as c0.
#
# Cells 0-39: verbatim from A34 (Perch ONNX + ProtoSSM + Tucker SED + Konbu
#             Head, produces submission_protossm.csv / _sed.csv / _head.csv)
# Cells E0-E3 (new): timm install, sys.path setup, NFNet inference via
#             birdclef2026.inference.predict_for_models, load OOF AUC
# Cell F (replaces A34 cell 40): 4-way RRF + per-class NFNet weight +
#             R1/R2/R3 (A34's mattia rescues, unchanged) + R4 (NFNet
#             diversity boost on trusted classes) + R5 (3+ branch consensus)

# %% [markdown]
# # BirdCLEF+ 2026: 0.941 Public LB with ONNX Perch Sequence Modeling and SED Blend
#
# This notebook documents the all-public CPU inference pipeline behind our current best confirmed BirdCLEF+ 2026 submission: **0.941 public LB**, submitted from `afr1ste/birdclef-2026-vyanktesh-sequence-v1` on 2026-05-02 UTC.
#
# The important point is not a single trick. The score comes from combining a strong Perch embedding backbone, lightweight sequence models trained inside the notebook, calibrated post-processing, and a distilled SED ensemble. The final submission is a **rank-average blend of ProtoSSM/Perch sequence predictions and Tucker Arrants' public distilled SED predictions**.
#
# ## Compliance and attribution
#
# This is an annotated public write-up of an all-public Kaggle notebook workflow. It uses only competition data, Kaggle-attached public datasets/kernels/models, and offline Kaggle notebook execution. Internet is disabled. No private dataset, unreleased checkpoint, hidden test label, external scraping, or team-private code is included.
#
# Primary code lineage and public inputs:
#
# | Component | Public source |
# | --- | --- |
# | Base notebook and ONNX Perch sequence pipeline | Vyanktesh Dwivedi, `birdclef-2026-onnx-perch-sequence-modeling` |
# | Distilled SED ONNX folds | Tucker Arrants, `bc2026-distilled-sed-public` |
# | Perch metadata | Jaejohn, `perch-meta` |
# | Perch ONNX export | Rishikesh Jani, `perch-onnx-for-birdclef-2026` |
# | TensorFlow wheel helper | Ashok205, `tf-wheels` |
# | Perch model | Google Bird Vocalization Classifier, TensorFlow2 Perch v2 CPU |
#
# The added markdown is meant to make the method auditable and easier to learn from. If you fork or reuse this notebook, please keep the same attribution chain visible.
#
# ## High-level pipeline
#
# 1. Load BirdCLEF taxonomy, sample submission, and train soundscape metadata.
# 2. Use the Perch model, preferably through ONNX Runtime, to produce window-level logits and embeddings.
# 3. Build train-window labels and lightweight sequence learners inside the notebook.
# 4. Apply calibration, confidence scaling, rank-aware scaling, adaptive temporal smoothing, and residual correction.
# 5. Run the public distilled SED ONNX ensemble on the same 5-second windows.
# 6. Rank-average the Perch/ProtoSSM branch with the SED branch and write `submission.csv`.
...
#
# Replaces the original 2-way ProtoSSM/SED rank blend with a 3-way rank blend (Proto / SED / Head, mix 0.50 / 0.35 / 0.15) and adds three rescue gates from mattiaangeli's better-blend (fake-only, Proto temporal continuity with fat-tail Student-t kernel, rare SED-spike). Head branch must be attached as a dataset containing `head_weights_train_audio.npz` (e.g. `konbu17/bird26-train-audio-head-v1`).
...
# %% [markdown]
# ## b07 — NFNet 4th branch (cells E0-E3)
#
# Install timm + load eB1b ckpt + run inference + load per-class OOF AUC.
# A34 already pulled in torch from the kaggle base image (used by ProtoSSM
# cell 26); we only need to add timm for the NFNet-L0 backbone build.

# %%
# ── b07 E0: install timm via ttahara wheels (--no-deps to avoid TF/onnxruntime conflicts) ──
import subprocess, sys, glob

_REQ_HITS = sorted(glob.glob(
    "/kaggle/input/**/birdclef-2026-download-wheels/requirements.txt",
    recursive=True,
))
assert _REQ_HITS, "ttahara wheel dataset not attached — check kernel-metadata.json kernel_sources"
_WHEELS = _REQ_HITS[0].replace("/requirements.txt", "/wheels")
subprocess.check_call([
    sys.executable, "-m", "pip", "install", "-q", "--no-deps",
    "timm",
    "--find-links", _WHEELS,
])
print("[b07-E0] timm installed (no-deps)")

# %%
# ── b07 E1: sys.path + audio config for birdclef2026 module ───────────
from pathlib import Path
...
_cfg = _yaml.safe_load((PKG_INPUT / "data.yaml").read_text())
...
print(f"[b07-E1] PKG_INPUT={PKG_INPUT}")
print(f"[b07-E1] sys.path[0]={sys.path[0]}")

# %%
# ── b07 E2: NFNet inference via birdclef2026.inference.predict ───────
import os
os.environ["CUDA_VISIBLE_DEVICES"] = ""  # Kaggle code-comp scoring is CPU-only
...
_fallback_cfg = load_yaml(WORK_CFG / "data.yaml")
_model, _target_cols, _audio_cfg = load_sed_checkpoint(
    _ckpt, _device, fallback_cfg=_fallback_cfg,
)
...
_predictions = predict_for_models(
    [_model], _rows,
    sample_rate=int(_audio_cfg.get("sample_rate", 32000)),
    clip_seconds=float(_audio_cfg.get("clip_seconds", 5.0)),
    device=_device, batch_size=4, amp_dtype="fp32",
    n_tta_crops=3,  # multi-crop TTA, mirrors val_n_crops=5 at training
)
print(f"[b07-E2] predictions: {len(_predictions)} row_ids")

# Scatter into dense matrix aligned with A34's meta_te["row_id"] ordering.
# meta_te["row_id"] is the canonical order used by submission_{protossm,sed,head}.csv.
_meta_row_ids = meta_te["row_id"].astype(str).values
pd_nfnet_raw = np.zeros((len(_meta_row_ids), len(_target_cols)), dtype=np.float32)
_missing = []
for i, rid in enumerate(_meta_row_ids):
    p = _predictions.get(rid)
    if p is None:
        _missing.append(rid)
    else:
        pd_nfnet_raw[i] = p
assert not _missing, f"[b07-E2] {len(_missing)} row_ids missing NFNet predictions, first 3: {_missing[:3]}"

# Reorder columns from ckpt's target_cols order → A34's PRIMARY_LABELS order.
_col_to_idx = {c: i for i, c in enumerate(_target_cols)}
_col_remap = np.array([_col_to_idx[c] for c in PRIMARY_LABELS], dtype=np.int64)
pd_nfnet = pd_nfnet_raw[:, _col_remap]
print(
    f"[b07-E2] pd_nfnet shape={pd_nfnet.shape}"
    f" range=[{pd_nfnet.min():.4e}, {pd_nfnet.max():.4e}]"
    f" mean={pd_nfnet.mean():.4e}"
)

# %%
# ── b07 E3: load per-class OOF AUC for the RRF NFNet weight ────────────
nfnet_oof_auc_raw = np.load(PKG_INPUT / "nfnet_oof_auc.npy")
assert nfnet_oof_auc_raw.shape == (len(_target_cols),), (
    f"[b07-E3] OOF AUC shape {nfnet_oof_auc_raw.shape} != ({len(_target_cols)},)"
)
# Same column reorder as pd_nfnet above so weights line up with PRIMARY_LABELS.
nfnet_oof_auc = nfnet_oof_auc_raw[_col_remap]
print(
    f"[b07-E3] OOF AUC: mean={nfnet_oof_auc.mean():.4f}"
    f" min={nfnet_oof_auc.min():.4f}"
    f" max={nfnet_oof_auc.max():.4f}"
    f" >0.9: {(nfnet_oof_auc > 0.9).sum()}/{len(nfnet_oof_auc)}"
    f" >0.95: {(nfnet_oof_auc > 0.95).sum()}/{len(nfnet_oof_auc)}"
)

# %% [markdown]
# ## b07 — Final blend (cell F replaces A34 cell 40)
#
# 4 branches:
#   pa  = Proto    (A34's ProtoSSM + Perch sequence model)
#   pb  = SED      (A34's Tucker distilled SED ONNX, 5-fold avg)
#   pc  = Head     (A34's Konbu konbu17 train_audio linear head)
#   pd_n = NFNet   (our eB1b NFNet-L0 + BG noise + cross-species mixup)
#
# Steps:
#   Layer 1: per-class rank percentile per branch (xa/xb/xc/xd)
#   Layer 2: 4-way Reciprocal Rank Fusion (RRF) with k=60, where NFNet's
#            contribution is scaled per-class by its OOF AUC trust:
#              w_d_class = clip((auc - 0.5) / 0.4, 0, 1.25)
#            Result is rank-normalized → pred ∈ [0, 1]
#   Layer 3a: A34's mattiaangeli rescues R1/R2/R3 unchanged (fake_only,
#             proto_cont, sed_only) — they operate on the new pred.
#   Layer 3b: R4 — NFNet diversity boost: when NFNet ranks the (row, class)
#             in top-5% but the other 3 average below 50%, AND NFNet's
#             OOF AUC > 0.9 on that class, blend 10% NFNet rank into pred.
#             This is the c0 design's main bet: NFNet's normalizer-free
#             architecture catches things BN-heavy models miss under
#             domain shift.
#   Layer 3c: R5 — strong consensus: when 3+ branches all rank the
#             (row, class) in top-10%, floor pred at 0.95.

# %%
# ── b07 cell F: 4-way RRF blend (Proto + SED + Head + NFNet) + R1-R5 ──
import numpy as np
import pandas as pd
...
# ---- 4-way base weights (A34's 0.45/0.40/0.15 scaled by 0.85, NFNet at 0.15) ----
W_PROTO  = 0.3825
W_SED    = 0.3400
W_HEAD   = 0.1275
W_NFNET  = 0.1500

# ---- A34 R1/R2/R3 knobs (unchanged) ----
FAKE_ONLY_THR   = 0.50
SED_LOW_THR     = 0.05
...
# ---- R4/R5 knobs (new in c0) ----
R4_RANK_HIGH_THR   = 0.95
R4_OTHERS_MEAN_THR = 0.50
R4_BLEND           = 0.10
R4_AUC_TRUST_THR   = 0.90   # only apply on classes where NFNet OOF AUC > this

R5_VOTE_RANK_THR   = 0.90
R5_VOTE_MIN        = 3
R5_BOOST_TO        = 0.95

# ---- load the three A34 branches written by earlier cells ----
...
assert cols == list(PRIMARY_LABELS), "A34 submission column order != PRIMARY_LABELS"

# Re-align sed/head to proto's row_id order (defensive; should already match).
...
# pd_nfnet was scattered using meta_te["row_id"] in E2, which is the same
# row_id ordering A34 used to build proto_df. Defensive shape check only.
assert pd_nfnet.shape == (len(proto_df), len(cols)), (
    f"pd_nfnet shape {pd_nfnet.shape} != proto_df ({len(proto_df)}, {len(cols)})"
)
...
pd_n = np.clip(pd_nfnet.astype(np.float32),        EPS, 1.0 - EPS)

# Per-class rank percentile within each branch.
...
xd = pd.DataFrame(pd_n).rank(axis=0, pct=True).to_numpy(np.float32)
...
# AUC ≤ 0.5  → weight 0    (NFNet random or worse on this class, no contribution)
# AUC 0.7    → weight 0.50
# AUC 0.9    → weight 1.00
# AUC ≥ 0.9 (capped at 1.25 for AUC = 1.0)
w_d_class = np.clip((nfnet_oof_auc - 0.5) / 0.4, 0.0, 1.25)[None, :]

base = (
    W_PROTO * _rrf_score(pa)
    + W_SED   * _rrf_score(pb)
    + W_HEAD  * _rrf_score(pc)
    + W_NFNET * w_d_class * _rrf_score(pd_n)
)
# Rank-normalize so the rest of the rescue logic operates on [0,1] percentiles.
pred = pd.DataFrame(base).rank(axis=0, pct=True).to_numpy(np.float32)

# ---- Layer 3a R1: A34 Proto fake-only rescue ----
fake_only = (pa > FAKE_ONLY_THR) & (pb < SED_LOW_THR)
pred = np.where(fake_only, (1.0 - FAKE_ONLY_BLEND) * pred + FAKE_ONLY_BLEND * xa, pred)

# ---- Layer 3a R2: A34 Proto temporal-continuity rescue (Student-t kernel) ----
...
# ---- Layer 3a R3: A34 rare local SED-spike rescue ----
sed_only = (
...
# ---- Layer 3b R4: NFNet diversity boost on trusted classes ----
nfnet_trusted = (nfnet_oof_auc > R4_AUC_TRUST_THR)[None, :]
r4 = (
    (xd > R4_RANK_HIGH_THR)
    & (((xa + xb + xc) / 3.0) < R4_OTHERS_MEAN_THR)
    & nfnet_trusted
    & (~fake_only) & (~proto_cont) & (~sed_only)
)
pred = np.where(r4, (1.0 - R4_BLEND) * pred + R4_BLEND * xd, pred)

# ---- Layer 3c R5: 3+ branch consensus boost ----
votes = (
    (xa > R5_VOTE_RANK_THR).astype(np.int8)
    + (xb > R5_VOTE_RANK_THR).astype(np.int8)
    + (xc > R5_VOTE_RANK_THR).astype(np.int8)
    + (xd > R5_VOTE_RANK_THR).astype(np.int8)
)
r5 = (votes >= R5_VOTE_MIN) & (pred < R5_BOOST_TO)
pred = np.where(r5, np.maximum(pred, R5_BOOST_TO), pred)
...
print(
    f"[b07-F] saved submission.csv: {final_sub.shape}"
    f"\n  RRF weights: PROTO={W_PROTO:.4f} SED={W_SED:.4f} HEAD={W_HEAD:.4f} NFNET={W_NFNET:.4f}"
    f"\n  rescues: fake_only={int(fake_only.sum())}"
    f"  proto_cont={int(proto_cont.sum())}"
    f"  sed_only={int(sed_only.sum())}"
    f"  r4={int(r4.sum())}"
    f"  r5={int(r5.sum())}"
)
