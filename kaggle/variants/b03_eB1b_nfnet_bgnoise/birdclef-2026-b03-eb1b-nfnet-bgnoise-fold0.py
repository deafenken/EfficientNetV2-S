# %% [markdown]
# # BirdCLEF 2026 — b03: eB1b NFNet-L0 + BG noise + cross-species mixup fold-0
#
# Single-fold inference using `outputs/exp/eB1b_nfnet_bgnoise/fold_0/swa.pt`.
# eB1b is eB1 (NFNet-L0 ImageNet) with two upgrades stacked on top of the
# baseline recipe:
#   - **W1 fix**: ESC50 ambient background noise mixed in at train time
#     (matches BirdCLEF 2025 winner recipe).
#   - **Cross-species mixup**: secondary-label aware mixup that respects
#     primary/secondary multi-label structure.
#
# **Why we submit this**: validate the W1 fix vs eB1 (LB 0.75). If LB lifts,
# BG noise + cross-species mixup are net-positive on the test soundscape
# domain (vs the XC-only val set where eB1 already topped 0.97). Pair this
# with b04 (eB1b + R1 pseudo-labels) to attribute pseudo gains separately.
