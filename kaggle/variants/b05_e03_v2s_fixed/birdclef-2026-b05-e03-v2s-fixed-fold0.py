# %% [markdown]
# # BirdCLEF 2026 — b05: e03 V2-S features_only + warmstart_strict + BG noise fold-0
#
# Single-fold inference using `outputs/exp/e03_v2s_fixed/fold_0/swa.pt`.
# e03 fixes the e01 V2-S baseline that collapsed on LB (b02 control was
# LB 0.43 despite XC val_auc 0.8553):
#   - **features_only=True** on the timm V2-S backbone (instead of full
#     classifier head — strips the ImageNet final layer mismatch).
#   - **warmstart_strict=True** on the BC2024 pretrained checkpoint (no
#     silent dropping of misshapen weights, surfaces real load failures).
#   - **BG noise** augmentation (ESC50 ambient mix, same as eB1b/W1).
#
# **Why we submit this**: validate the V2-S fix vs e01 (LB 0.43). If b05
# LB > e01 LB by a wide margin, the warmstart + features_only fix is the
# real win and we can revive the V2-S pretrained branch as an ensemble
# partner for the eB1* NFNet line.
...
PKG_INPUT = Path("/kaggle/input/birdclef2026-e03-pkg-ckpt")
